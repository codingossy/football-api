...
const Standings = () => {
  ...
  return (
    <div className="standings-container px-5 lg:px-0">
      <div className="select-fields flex flex-col lg:flex-row  justify-start lg:justify-center">
        <select className='p-3 text-xs lg:text-sm bg-black text-white'
          name="select-league"
          id="select-league"
        ...
        <select
          name="select-year"
          className='p-3 text-xs lg:text-sm bg-gray-500 text-white'
          id="select-year"
      ...
      <div className="ranking-div text-center flex items-center justify-center flex-col m-5">
        {loading ? (
          <Loader type="Puff" color="#00BFFF" height={80} width={80} />
        ) : (
          data?.map((data, index) => (
            <div key={index} className="flex items-center justify-center gap-x-4">
              <h1 className='flex items-center gap-y-4 gap-x-4'>
                <span className='flex gap-y-4 gap-x-4 items-center justify-center'>
                  {`${index + 1}.`}
                  <img
                    src={data.team.logos[0]?.href}
                    alt="#"
                    className="logo-small w-5"
                  />
                </span>{" "}
